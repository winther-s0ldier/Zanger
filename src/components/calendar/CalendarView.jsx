...
const CalendarView = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await axios.get("http://54.167.18.161/api/calendar", {
        headers: {
  ...
  const handleAddEvent = async (eventData) => {
    try {
      ...
      const response = await axios.post(
        "http://54.167.18.161/api/calendar",
        eventData,
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
  ...
  const handleDelete = async (eventId) => {
    try {
      await axios.delete(`http://54.167.18.161/api/calendar/${eventId}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
  ...
  const handleGoogleSync = async () => {
    try {
      const response = await axios.post(
        "http://54.167.18.161/api/auth/google/calendar/url",
        {},
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
