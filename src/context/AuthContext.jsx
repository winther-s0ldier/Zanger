...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch("http://54.167.18.161/api/auth/login", {
        method: "POST",
        headers: {
  ...
  const googleLogin = async (userInfo) => {
    try {
      ...
      const response = await fetch("http://54.167.18.161/api/auth/google", {
        method: "POST",
  ...
  const register = async (name, email, password) => {
    try {
      const response = await fetch("http://54.167.18.161/api/auth/register", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
